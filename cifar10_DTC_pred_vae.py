...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrain_dir', type=str, default='./data/experiments/cifar10_vae_classify/vae_cifar10.pth')
    parser.add_argument('--dataset_root', type=str, default='./data/datasets/CIFAR/')
    parser.add_argument('--exp_root', type=str, default='./data/experiments/')
    parser.add_argument('--model_name', type=str, default='resnet18')
    parser.add_argument('--save_txt_name', type=str, default='result.txt')
    parser.add_argument('--model_path', type=str, default='./data/experiments/cifar10_DTC_vae/PI-VAE/vae.pth')
    ...
    train_loader = CIFAR10Loader_VAE(root=args.dataset_root, batch_size=args.batch_size, split='train', labeled=False, aug='twice', shuffle=True)
    eval_loader = CIFAR10Loader_VAE(root=args.dataset_root, batch_size=args.batch_size, split='train', labeled=False, aug=None, shuffle=False)
    #
    # model = ResNet(BasicBlock, [2,2,2,2], 5).to(device)
    vae_base = VAE(128,32).to(device)
    # vae_base.load_state_dict(torch.load(args.model_path), strict=False)
    ...
    # vae_base.load_state_dict(torch.load(args.pretrain_dir), strict=False)
    ...
    print('plotting TSNE')
    model.linear= Identity()
    labelled_loader = CIFAR10Loader_VAE(root=args.dataset_root, batch_size=args.batch_size, split='train', labeled=True, aug=None, shuffle=False)
    #

    # plot_tsne(eval_loader, model, device,n_clusters=5, filename = 'tsne_test.png')
    plot_tsne_train(labelled_loader, model, device,n_clusters=5, filename = 'tsne_train_vae.png')
